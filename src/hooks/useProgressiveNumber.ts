...
export const useProgressiveNumber = (
  ...
  useEffect(() => {
    const timeout = setTimeout(() => {
      ...
      if (progress === 1) { setCurrent(target) }
      else { setCurrent(lerp(initial, target, easeOutCubic(progress))); setCurrentStep(currentStep + 1) }
    }, delay)
    return () => clearTimeout(timeout)
  }, [delay, currentStep, target])

  return [current.toFixed(decimals), setValue]
}
